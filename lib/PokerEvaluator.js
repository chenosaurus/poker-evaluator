...
module.exports = {
  ...
  //evaluate 3 card hand, using na
  eval3: function(cards) {
    //simply and sort
    cards = cards.map(function(card) {
      return Math.ceil(card/4);
    });

    cards.sort(function(a,b) { return a-b }).reverse();

    var address = ((cards[0] << 8) + (cards[1] << 4) + cards[2]) * 2;
    console.log(cards, address)
    var p = this.ranks3.readUInt16LE(address);
...
var ranksFile3Cards = path.join(__dirname, '../data/3CardHandRanks.dat');
module.exports.ranks = fs.readFileSync(ranksFile);
module.exports.ranks3 = fs.readFileSync(ranksFile3Cards);
